...
class ChunkConfig(models.Model):
    ...
    class Meta:
        verbose_name = "Chunk Configuration"
        verbose_name_plural = "Chunk Configurations"
        ordering = ["-updated_at"]
...
class Chunk(models.Model):
    ...
    def previous(self):
        ...
        return (
            Chunk.objects.filter(
                python_file=self.python_file, updated_at__lt=self.updated_at
            )
            .order_by("-updated_at")
            .first()
        )
    ...
    def next(self):
        ...
        return (
            Chunk.objects.filter(
                python_file=self.python_file, updated_at__gt=self.updated_at
            )
            .order_by("updated_at")
            .first()
        )
...
class ChunkDescription(models.Model):
    chunk = models.ForeignKey(
        Chunk, on_delete=models.CASCADE, related_name="descriptions"
    )
